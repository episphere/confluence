                ...
                <div class="main-summary-row white-bg div-border">
                    <ul class="nav nav-pills">
                        ${
                            ...
                            ` : `
                                <li class="nav-item">
                                    <a class="nav-link black-font font-size-14" href="#data_exploration/dictionary">
                            ...
                            ${accessToUpload ? (` 
                                ...
                                <!--${
                                    location.origin.match(applicationURLs.prod) ? `` : `
                                        <li class="nav-item">
                                            <a class="nav-link black-font font-size-14" href="#data_exploration/subset">
...
export const dataSummaryMissingTemplate = async () => {
    ...
    data.forEach((dt) => {
        ...
        if (dt["study"] && studies[dt["Consortia"]][dt["study"]] === undefined)
        studies[dt["Consortia"]][dt["study"]] = {};
    });
            ...
            <div class="col white-bg div-border align-left font-size-17" style="padding: 0.5rem;" id="listFilters">
                ...
                ${
                    initialSelection.length > 0 ? `
                        ...
                        <span class="font-bold">Variable: </span>${initialSelection[0]} 
                        ${
                            initialSelection.length > 1 ? `and <span class="other-variable-count">${initialSelection.length - 1} other</span>` : ``
                        }
                    ` : ``
                }
            </div>
...
const renderMidsetFilterData = (
    ...
    ancestory.forEach((anc) => {
        template += `
            <option value="${anc}" ${anc === "All" ? "selected" : ""}>
                ${anc.replace(new RegExp("ethnicityClass_", "i"), "")}
            </option>
        `;
    });
    ...
    template += `
        <li class="list-group-item no-list-style filter-list-item d-flex justify-content-between align-items-center">
            <div> 
...
const filterMidsetData = (data) => {
    ...
    if (studiesSelection.length > 0 || consortiaSelection.length > 0) {
        newData = newData.filter((dt) => studiesSelection.indexOf(dt["study"]) !== -1 || consortiaSelection.indexOf(dt["Consortia"]) !== -1);
    }
    if (status !== "All") {
    ...
    document.getElementById("listFilters").innerHTML = `
        ...
        ${
            selectedVariables.length > 0 ? `
                <span class="vertical-line"></span>
                <span class="font-bold">Variable: </span>${selectedVariables[0]}
                ${
                    selectedVariables.length > 1 ? `
                        and <span class="other-variable-count"> ${selectedVariables.length - 1} other</span>` : ``
                }
            ` : ``
        }
        ${
            studiesSelection.length > 0 ? `
                <span class="vertical-line"></span>
                <span class="font-bold">Study: </span>${studiesSelection[0]}
                ${
                    studiesSelection.length > 1 ? `
                        and <span class="other-variable-count">${studiesSelection.length - 1} other</span>` : ``
                }
            ` : ``
        }
    `;
    ...
    if (data.length > 0) {
        ...
        template += `
            <tr class="midset-header">
                ...
                <th class='bar-chart-cell' colspan="${Object.keys(headerCount).length}">
    ...
    <tbody>
        ...
        acceptedVariables.forEach((variable, index) => {
            ...
            if (index === acceptedVariables.length - 1)
                template += `
                    <td class="missing-column">
                        ${numberWithCommas(set0)}
                    </td>
